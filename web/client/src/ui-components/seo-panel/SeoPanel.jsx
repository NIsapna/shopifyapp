...
import { enableEditMode, closeSeoPanel, setSelectedBlogData ,disableEditMode} from "../../store/blogSlice";
...
const SeoPanel = () => {
  ...
  const { showSeoPanel, selectedBlogData, editMode } = useSelector(
    (state) => state.blog
  );
  // console.log("selectedBlogData", showSeoPanel,selectedBlogData);
  ...
  const hasIssues = allAnalyses.some(a => a.status !== "good");
  ...
  return (
    ...
    >
      <BlockStack gap={200} >
        ...
        {hasIssues && (
          <Box paddingBlock={100} align='center' >
            <Button
              variant="primary"
              disabled={editMode || isPlanPending || isPlanExpired}
              onClick={() => dispatch(enableEditMode("all_fields"))} // trigger single editor
            >
              Fix All Issues
            </Button>
          </Box>
